...
export default function ChecklistPersonas() {
  const [controles, setControles] = useState(() => {
    return JSON.parse(localStorage.getItem("iso27001_checklist_personas")) || [];
  });
  ...
  useEffect(() => {
    localStorage.setItem("iso27001_checklist_personas", JSON.stringify(controles));
  }, [controles]);
  ...
  return (
    <div className="checklist-page">
      <h1>Checklist - Dominio Personas</h1>
      <p>Controles A.6.1 – A.6.8: selección, capacitación, confidencialidad, trabajo remoto, etc.</p>

      <div className="checklist-form">
        <input type="text" placeholder="Código (ej. A.6.1)" value={nuevo.codigo} onChange={(e) => setNuevo({ ...nuevo, codigo: e.target.value })} />
      ...
      <table className="checklist-table">
        <thead>
          <tr>
            <th>Código</th><th>Descripción</th><th>Estado</th><th>Evidencia</th>
            <th>Responsable</th><th>Fecha</th><th>Observaciones</th><th>Acción</th>
          </tr>
        </thead>
        <tbody>
          {controles.map((c, i) => (
